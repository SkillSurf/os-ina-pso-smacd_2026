...
def vgs_error(vgs_guess, target_gm_id, L, VSB):
    with silence_stdout():
...
def survivability_test(particle, verbose=False):
    ...
    where L_1_idx and L_2_idx are indices into L_DISCRETE_VALUES
    """
    gm1, gm2, L_1_idx, L_2_idx, ID = particle
    
    # Convert discrete indices to actual L values
...
def generate_particle(cont_bounds, n_L_values, max_attempts=1000):
    """
    cont_bounds = [gm1_bounds, gm2_bounds, ID_bounds]
    n_L_values = # of discrete L values available
    """
    for attempt in range(max_attempts):
        ...
        # Generate discrete variable indices
...
class PSOmv:
    ...
    def __init__(self, cont_bounds, n_L_values, n_particles=50, max_iterations=100, max_velocity_updates=5):
        ...
        self.velocities = np.zeros((n_particles, self.n_cont))  # Only continuous variables have velocity
        ...
        # Adaptive parameter initialize 
        self.w_bar = 0.5  # Average inertia weight
        self.alpha_bar = 0.5  # Average alpha
        self.param_set = []  # SetP
        self.S = 0  # Counter
        
        # GLOBAL probability distributions for discrete variables 
    ...
    def initialize_swarm(self):
        ...
        # Initialize velocities for continuous variables 
    ...
    def continuous_reproduction(self, particle_idx, w):

        sorted_indices = np.argsort(self.pbest_fitness)[::-1]  # Worst to best
        particle_rank = np.where(sorted_indices == particle_idx)[0][0]
        
        # Generate new continuous variables
        new_cont_vars = np.zeros(self.n_cont)
        

        for i, cont_idx in enumerate(self.continuous_indices):

            r = np.random.randint(particle_rank, self.n_particles)
            learning_idx = sorted_indices[r]

            c = 2.0
            rand = np.random.random()
            pbest_learning = self.pbest_positions[learning_idx, cont_idx]
            current_pos = self.positions[particle_idx, cont_idx]
            
            self.velocities[particle_idx, i] = (
                w * self.velocities[particle_idx, i] + 
                c * rand * (pbest_learning - current_pos)
            )
    ...
    def discrete_reproduction(self, particle_idx):
        """
        Sample from GLOBAL probabilities
        """
    ...
    def generate_offspring(self, particle_idx, w, alpha):

        for attempt in range(self.max_velocity_updates):
        
            new_cont_vars = self.continuous_reproduction(particle_idx, w)
            ...
            # If failed, update velocity again and retry 
        
      
        print(f"    Particle {particle_idx+1}: Failed after {self.max_velocity_updates} attempts, generating new particle...")
        particle, area, specs = generate_particle(self.cont_bounds, self.n_L_values)
        
        if particle is not None:
            return particle, area, specs, True
        else:
    ...
    def optimize(self):
        ...
        for iteration in range(self.max_iterations):
            ...
            _, alpha_iter = self.adaptive_parameter_tuning()
            

            self.update_discrete_probabilities(alpha_iter)
            

            for i in range(self.n_particles):
                w_i, alpha_i = self.adaptive_parameter_tuning()
                
            
                offspring, area, specs, success = self.generate_offspring(i, w_i, alpha_i)
                
                if not success:
                    continue

                
                # update particle if better
                if area < self.fitness[i]:
                    self.positions[i] = offspring
                    self.fitness[i] = area
                
                # update pbest and parameters
                if area < self.pbest_fitness[i]:
                    self.pbest_fitness[i] = area
                    self.pbest_positions[i] = offspring.copy()
                    
                
                    self.update_parameters(w_i, alpha_i)
            ...
            print(f"  Global Best: {self.gbest_fitness:.2f} μm²")
            print(f"  Average: {avg_fitness:.2f} μm²")
            print(f"  w̄={self.w_bar:.3f}, ᾱ={self.alpha_bar:.3f}")
        
        # Line 24: Return gbest
    ...
    def get_results(self):
        ...
        results = {
            ...
            'final_probabilities': {
                'L_1_probs': self.discrete_probs[0, :],
                'L_2_probs': self.discrete_probs[1, :]
            }
        }
    ...
    def plot_convergence(self, save_path='pso_convergence.png'):
        ...
        if len(self.param_set) > 0:
            ...
            plt.plot(w_history, 'b-', alpha=0.5, linewidth=1, label='w')
            plt.plot(alpha_history, 'r-', alpha=0.5, linewidth=1, label='α')
        ...
        # Plot 4: Final discrete probabilities
        ...
        plt.xlabel('Discrete L Values (μm)', fontsize=11)
        plt.ylabel('Probability', fontsize=11)
        plt.title('Final Discrete Probabilities', fontsize=12, fontweight='bold')
        plt.xticks(x_pos, [f'{v:.2f}' for v in L_DISCRETE_VALUES], rotation=45, ha='right', fontsize=8)
        plt.legend(fontsize=9)
...
def main():
    ...
    print(f"\nDiscrete Variables:")
    print(f"  L_1, L_2 ∈ {{{', '.join([f'{v:.2f}' for v in L_available])}}} μm")
    ...
    if results:
        ...
        # Plot and save
        pso.plot_convergence('pso_mixedvar_convergence.png')
...
def save_results_to_file(results, opt_time):
    ...
    with open('pso_mixedvar_results.txt', 'w', encoding='utf-8') as f:
