...
C {devices/code.sym} -380 -10 0 0 {name=TT_MODELS
only_toplevel=true
...
"
spice_ignore=false}
C {devices/code_shown.sym} -610 -80 0 0 {name=NGSPICE only_toplevel=true 
value="
.param temp=25
.option savecurrents
Vvdd VDD 0 1.8
...
C {code.sym} -380 -180 0 0 {name=GAIN only_toplevel=false
value="
.control
  tran 0.1m 2m
  save all

  let v_in = v(vip) - v(vin)
  let v_out = v(vout)

  plot v_in
  plot v_out

  meas tran v_in_pk  PP v_in from=1m to=2m
  meas tran v_out_pk PP v_out from=1m to=2m

  let gain = v_out_pk / v_in_pk
  print gain
